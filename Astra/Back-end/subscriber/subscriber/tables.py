...
class SlaveGateway(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    gatewayid = Column(String(20), unique=True)
    master_id = Column(ForeignKey(MasterGateway.id, ondelete='CASCADE'))
    lastseen = Column(DateTime(timezone=True), server_default=func.now())
...
class EnergyTracking(Base):
    __tablename__ = 'energy_energytracking'
...
class Energy_VehicleTracking(Base):

    __tablename__ = 'energy_vehicletracking'

    id = Column(Integer, primary_key=True, autoincrement=True)
    tag = Column(String(40))
    vehicle_status = Column(Boolean)
    timestamp = Column(DateTime(timezone=True))


class PassiveAsset(Base):
    __tablename__ = 'energy_passiveasset'
