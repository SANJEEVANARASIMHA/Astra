...
export const showMessage = (self_key,interval, error, success, msg) => {
    clearTimeout(self_key.messageTimeout);
    self_key.setState({ error: error, success: success, message: msg });
    if (interval) {
        self_key.messageTimeout = setTimeout(() => {
            self_key.setState({ error: false, success: false, message: "" });
        }, 5000)
    }
}